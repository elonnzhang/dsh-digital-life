...
export function AgentPresetSelector({ load, select, records, selectLife, t, }) {
    ...
    return (_jsxs("div", { className: css.root, children: [_jsx(Menu, { className: `${css.seatWrap} ${css.presetWrap}`, open: presetOpen, onClose: () => {
                    ...
                    id: option.id,
                    label: (_jsxs("span", { className: css.item, children: [_jsx("span", { className: css.itemName, children: option.name }), _jsx("span", { className: css.itemDesc, children: option.description ?? t("presetNoDescription") })] })),
                    ...
                    id: record.id,
                    label: (_jsxs("span", { className: css.item, children: [_jsx("span", { className: css.itemName, children: record.name }), _jsxs("span", { className: css.itemDesc, children: [record.description, " \u00B7 ", categoryLabel(record, t)] })] })),
                })), selectedId: life, onSelect: (id) => {
                    setLifeOpen(false);
                    setLife(id);
                    void selectLife(id);
                }, align: "start", portal: true, anchor: _jsxs("button", { type: "button", className: `${css.seat} ${life === "" ? css.required : ""}`, "aria-haspopup": "menu", "aria-expanded": lifeOpen, title: life === "" ? t("chooseLifeRequired") : chosenLife?.name, onClick: () => {
                        setLifeOpen((value) => !value);
                    }, children: [_jsx("span", { className: css.lifeIcon, children: "\uD83E\uDDE0" }), _jsx("span", { className: css.seatLabel, children: chosenLife?.name ?? t("chooseLife") }), _jsx(IconChevronDownOutline14, { className: css.chevron })] }) }))] }));
}
