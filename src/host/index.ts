...
export const Config: z<DigitalLifeSettings> = z.object({
...
function enforceReadOnlySandbox(session: Session): void {
  for (let index = session.events.length - 1; index >= 0; index -= 1) {
    const event = session.events[index] as { type?: unknown; data?: { mode?: unknown } } | undefined;
    if (event?.type === "sandbox/mode") {
      if (event.data?.mode === "read-only") return;
      break;
    }
  }
  (session.append as unknown as (type: string, data: { mode: string }) => void)(
    "sandbox/mode",
    { mode: "read-only" },
  );
}
...
export function apply(ctx: Context, entry: DigitalLifeSettings): void {
  let source = (): DigitalLifeSettings => entry;
